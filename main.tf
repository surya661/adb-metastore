...
# C:\Users\vvardhan1\Terraform\POC UC Setup\Modules and metastore
...
module "metastore_and_users" {
  ...
  databricks_access_connector_name = "my-databricks-access-connector"
  access_connector_identity_type   = "SystemAssigned" # or "UserAssigned"
  identity_ids                     = []               # List of UserAssigned MI IDs if needed, empty for SystemAssigned
  access_connector_tags = {
    environment = "dev"
    owner       = "myteam"
  }

  # Storage account (as needed by storage module)

  storage_account_name              = "myunicatalogsa"
  ...
  # Add any other required inputs for your use case
  log_analytics_workspace_name = "law-poc"
  log_analytics_workspace_rg_name = "law-sub"
}
